...
const MatchesTabs = ({ active, navigate }) => {
  const activeClass = tab =>
    active === tab ? "btn-dark text-light" : "btn-light";

  return (
    <div className="row ml-0 mr-0">
      <div className="column col-6 pl-0 pr-0">
        <a
          className={`w-100 btn pt-2 pr-4 pb-2 pl-4 ${activeClass("current")}`}
      ...
      <div className="column col-6 pl-0 pr-0">
        <a
          className={`w-100 btn pt-2 pr-4 pb-2 pl-4 ${activeClass("upcoming")}`}
...
const withMatchesLayout = WrappedComponent => props => {
  ...
  return (
    <div>
      <h1>Matches</h1>
      {props.showUpcoming && <MatchesTabs active={tab} navigate={navigate} />}
